...
from transformers import BertTokenizer, TFBertForSequenceClassification, BertForSequenceClassification, AdamW, \
...
class SentimentBERT:
    ...
    def evaluate(self, dataloader, positive_ratio):
        positive_size = int(positive_ratio * TOTAL_DATASIZE)
        negative_size = TOTAL_DATASIZE - int(positive_ratio * TOTAL_DATASIZE)
        y_true = np.append(np.zeros(positive_size), np.ones(negative_size))
