...
from LanguageFineTuning.BERT_sentiment_dataset import SentimentDataset
from BERT_sentiment_model import SentimentBERT
...
parser.add_argument('--path', default='weights_0.4/', type=str, help="Weights path")
parser.add_argument('--ratio', default=0.5, type=float, help="Positive samples proportion")
...
def train(train_file, epochs=20, output_dir="weights/", positive_sample=args.ratio):
    ...
    dataloader = dt.prepare_dataloader(train_file, positive_sample, sampler=RandomSampler)
...
def evaluate(test_file, model_dir="weights/", positive_sample=args.ratio):
    predictor = SentimentBERT()
    predictor.load(model_dir=model_dir)

    dt = SentimentDataset(predictor.tokenizer)
    dataloader = dt.prepare_dataloader(test_file, positive_sample)
    score = predictor.evaluate(dataloader, positive_sample)
    print(score)
...
if __name__ == '__main__':
    if args.train:
        ...
        train(args.train_file, epochs=10, output_dir=args.path, positive_sample=args.ratio)
