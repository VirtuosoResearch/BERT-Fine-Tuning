...
class SentimentDataset:
    ...
    def prepare_dataloader(self, filename, data_positive, sampler=None):
        positive_size = int(data_positive*TOTAL_DATASIZE)
        negative_size = TOTAL_DATASIZE-int(data_positive*TOTAL_DATASIZE)
        data = self._read_imdb_data(filename)
        data_positive = random.sample(data[:12500], positive_size)
        data_negative = random.sample(data[12500:], negative_size)
        dataX = data_positive + data_negative
        dataY = np.append(np.zeros(positive_size), np.ones(negative_size))
        sentences_with_labels = zip(dataX, dataY.tolist())
